...
First, I'd like to briefly list the services I use for hosting my personal website. I'm choosing to gloss over the exact steps that I followed in order to set everything up because these tasks are pretty straight-forward and well-documented. Below are some of the tutorials I followed:
...
As mentioned in my previous post, I used the [create-react-blog](https://github.com/jamesknelson/create-react-blog) template as a basis from which I built my personal website. A large portion of my quick deployments heavily depends on how this template is configured. For those following along, the instructions will be a lot simpler if you're using a React application with a [similar configuration](https://github.com/shawncruz/shawncruz.com/blob/master/package.json).
...
This tutorials assumes the following are already set up:
- [Virtual machine running Ubuntu](https://www.digitalocean.com/community/tutorials/initial-server-setup-with-ubuntu-18-04)
- [Web server](https://www.digitalocean.com/community/tutorials/how-to-install-nginx-on-ubuntu-18-04)
- [React web application](https://github.com/jamesknelson/create-react-blog)

# Implementation Overview

From a high level, the deployment pipeline will use a bare repository with a `post-receive` hook to deploy changes to a React web application.

## Bare Repository

The first thing we need to do in order to set up the deployment pipeline is to create a bare repository on a virtual machine.

### What is a Bare Repository?

A bare repository acts as a centralized repository, encapsulated away from `master`, where developers can collaboratively push changes. This is distinct from a typical remote branch, because by default a bare repository doesn't have a working tree, and therefore cannot function as a place where changes can be checked in and commit. It is also important to not pull changes from within a bare repository, because a bare repository conventionally indicates that it should only receive changes.
...
In general though, there is no reason to pull changes from a remote repository. This would require us to manually SSH into the machine, pull the correct changes, then rebuild the application. Below, I explain how to automate these steps.

### Create a Bare Repository

Creating a bare repository is relatively straight forward. In our virtual machine running Ubuntu:

```bash
# Create a directory where our bare repository will be located, then change into that directory
$ mkdir <name-of-bare-repo> && cd "$_"
...
On our local machine, add a new remote pointing to the bare repository we just created--I suggest naming it `production` to distinguish it from our standard remote `origin`:
```bash
# This remote can be called anything other than "origin"
$ git remote add production <server-url>:<name-of-bare-repo>
# Check to make sure that our remote looks good
$ git remote -v
# Should output something like
production <server-url>:<name-of-bare-repo> (fetch)
production <server-url>:<name-of-bare-repo> (push)
```

This indicates that we can fetch changes from and push changes to that repository.

Our bare repository will only act as a means of tracking the git commit history. Our changes will be checked out in a separate folder--from where they'll be served. The benefit of this is cleanliness, security, and a matter of separating any git-related data from the actual project files our application is built from.

### Add Web Root

From our virtual machine, add a directory from which to checkout changes. The web server should point to this directory when serving our website. We'll add this directory in the `/var/www` directory:
```bash
# Assumes that we set up server blocks pointing to this directory
$ mkdir /var/www/<name-of-web-app>
```
...
To accomplish this, we use a `post-receive` hook. A `post-receive` hook is a git hook that triggers downstream processes _after_ incoming changes have been received by a repository. For this reason we utilize this hook for rebuilding and redeploying our React web application.
...
GIT_DIR=/path/to/<name-of-bare-repo>
WORKING_TREE=/var/www/<name-of-web-app>
...
```
 ...
 Then, we checkout the new changes into our `WORKING_TREE` in our web root from our bare repository:

```bash
# This command checks out the changes from our repository (GIT_DIR) into our web root (WORKING_TREE)
$ git --work-tree=WORKING_TREE --git-dir=GIT_DIR checkout -f
```
...
Will run the `build` script specified in `package.json` (see: [package.json](https://github.com/shawncruz/shawncruz.com/blob/master/package.json)). This is targeted to what I have in my react application, but can be swapped for any other command that builds a React application.

Each command's output is echoed to the client so that progress, success, or failure can be tracked.
...
In order to verify that this deployment pipeline works, from your local machine, try pushing a change to the remote we created before. The messages echoed back will indicate the step by step procedure that occurs while running the `post-recieve` hook; ultimately indicating success or failure. If all goes well, the whole run should looks something like:

```bash
# Make and commit a visual change
$ git commit -am "Testing deployment pipeline"
# Push to remote
$ git push production
...
remote: # Depending on whether or not your package.json file was updated, the next section will either be blank or filled with npm install messages
remote: # Messages from `build` script
To <server-url>:<name-of-bare-repo> # This is the remote we created before
   <prev-SHA-1>..<curr-SHA-1>  master -> master
```

Upon success, refresh your live web page to see the changes applied instantly!

## Conclusion

Quickly recapping the deployment pipeline we've just implemented, we created a bare repository to which our changes can be push, and a `post-receive` hook that will checkout the incoming changes to the web root directory, then rebuild the web application.

In practice, I've found this to be extremely useful for a single developer. There's something very satisfying--and perhaps potentially dangerous--about being able to deploy changes so quickly to the web. While this method of development works for me, I'm curious to see if it resonates with anyone else. I'm particularly interested in knowing at what point this method of deployment fails to scale.