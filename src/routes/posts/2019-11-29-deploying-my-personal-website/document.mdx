...
In order to verify that this deployment pipeline works, from your local machine, try pushing a change to the bare repository we just created. The messages echoed back to will indicate the step by step procedure that occurs while running the `post-recieve` hook; ultimately indicating success or failure. If all goes well, the whole run should looks something like:

```bash

```

Upon success try refreshing your live web page to see the changes applied in realtime!
